...
TLAUNCHER_AUTH_URLS = [
    "https://auth.tlauncher.org/authenticate",
    "https://auth.tlauncher.org/login",
    "https://auth.tlauncher.org/auth"
]
...
class CatClientHDRV0(tk.Tk):
    ...
    def login(self):
        """Authenticate with TLauncher using multiple potential endpoints."""
        ...
        for url in TLAUNCHER_AUTH_URLS:
            try:
        ...
        # If all URLs fail
        messagebox.showerror("Error", "Authentication failed: Unable to find a valid authentication endpoint.")
        self.logged_in = False
